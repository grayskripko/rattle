...
#
# Time-stamp: <2014-04-27 08:04:39 gjw>
#
# WEATHER DATASET
#
...
acquireWeatherData <- function(write.to.file=FALSE)
{
  ...
  sites <- read.csv("site-numbers.csv")
  ...
  save(locationsAUS, file="locationsAUS.RData", compress="xz")
  ...
  save(weather, file="weather.RData", compress="xz")
...
updateWeatherLocation <- function(name, stnID, write.to.file, backto=14)
{
  ...
  for (f in dir(".", pattern=sprintf("^IDCJDW%s.*.csv$", as.character(stnID))))
  {
    # 100125 Calculate the amount of skip.

    skip <- as.integer(system(sprintf("grep -v '^,' %s | wc -l", f), intern=TRUE))

    # Read the CSV file.
    
    x <- read.csv(f, skip=skip, check.names=FALSE)
  ...
  if (write.to.file)
  {
    ...
    eval(parse(text=sprintf('save(weather%s, file="weather%s.RData", compress="xz")',
                 name, name)))
...
#  write.arff(weatherAUS, "weather.arff")
  save(weatherAUS, file="weatherAUS.RData", compress="xz")
